...
*  SELECT eastl~anlage, eastl~ab, eastl~bis, equi~sernr
*       INTO TABLE @DATA(lt_actieve_meters)
*       FROM eastl AS eastl
*       ON lt_aansluiting~anlage EQ eastl~anlage
...
*       WHERE eastl~ab  LE @vandaag() "bij rapportage: datum selectiescherm
*         AND eastl~bis GE @vandaag().
...
*    WITH KEY anlage = <fs_data>-analge
